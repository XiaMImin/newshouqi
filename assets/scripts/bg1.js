...
cc.Class({
    ...
    onLoad() {
        ...
        this.node.on(cc.Node.EventType.TOUCH_END, function (event) {
            ...
            if (self.node.name == 'kong') {
                ...
                // console.log(self.game.chessList);
                    ...
                    if (self.color == self.game.player) {
                        if (self.isSelect) {
                            ...
                            if (self.game.selectChess == null) {
                                ...
                                self.game.selectChess.getComponent('bg1').deSelect();
                        ...
                        //
                        if (self.game.selectChess != null && self.game.selectChess.getComponent('bg1').color == self.game.player) {
                            console.log('移动');
                            // console.log()
    ...
    move: function () {
        ...
        if (this.tag == 'chess') {
            ...
            if ((this.lv < ab.lv&&!(this.lv == 1 && ab.lv == 8)) || this.lv == 8 && ab.lv == 1) {
                ...
                if (this.hp <= 0) {
                    ...
                    this.game.hpEffect(this.node.getPosition(), 'hp-' + hp3)
                    // node1.runAction(blink)
                    this.node.runAction(blink)
                }
                if (ab.hp <= 0) {
                    ...
                    this.game.hpEffect(node1.getPosition(), 'hp-' + hp4)
                    // node1.runAction(blink)
                    // this.node.runAction(blink)
                }
                ...
                if (this.hp <= 0) {
                    ...
                    this.node.runAction(blink)
                    this.game.hpEffect(this.node.getPosition(), 'hp-' + hp4)
                }
                if (ab.hp <= 0) {
                    this.game.hpEffect(node1.getPosition(), 'x_X')
                    // this.addNode(node1);
                    ...
                    // node1.runAction(blink)
                    this.game.hpEffect(node1.getPosition(), 'hp-' + hp3)
                }
                //    node1.setPosition(this.node.getPosition());
                // this.node.setPosition(node1.getPosition());
            }
        ...
        if (ab.hp <= 0) {
            ...
            setTimeout(function () {
                // this.game.chessList[ab.index].destroy();
                node1.destroy();
            }.bind(this), 5000)
        }
