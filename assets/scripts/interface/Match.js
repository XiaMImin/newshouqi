...
cc.Class({
    ...
    onLoad: function () {
        ...
        this.avatar(self.playerOneAvatar,GLB.avatar)
    ...
    onEvent:function (event) {
        ...
        switch (event.type) {
            ...
            console.log('进入别人的房间通知')
            console.log(eventData.userInfoList)
             this.joinRoom(eventData.userInfoList);
                break;
            case msg.MATCHVS_JOIN_ROOM_NOTIFY:
            console.log('其他人加入房间通知')
                console.log(eventData.roomUserInfo)
            ...
            case msg.MATCHVS_NETWORK_STATE_NOTIFY:
                if (eventData.netNotify.state === 1) {
                    engine.prototype.kickPlayer(eventData.netNotify.userID,"你断线了，被踢出房间");
                }
                break;
        }
    ...
    joinRoom: function (userInfoList) {
        ...
        this.labelRoomID.string = '房间号：'+userInfoList.roomID;
        GLB.roomID = userInfoList.roomID;
        this.playerNameOne.string = GLB.userID;
    ...
    avatar:function(node,avatar){
        console.log('加载头像'+avatar+node)
        if (typeof(wx) !== 'undefined') {
            ...
            cc.loader.load(avatar, function (err, res) {
                if (err) {
